...
class ProdutoService {
  ...
  async atualizarProduto(id, dados) {
    try {
      ...
      const produtoExistente = await this.produtoRepository.buscarPorId(
        parseInt(id),
      );
      ...
      produtoExistente.descricao =
        dados.descricao || produtoExistente.descricao;
      ...
      // Salvar no repositório
      return await this.produtoRepository.atualizar(
        parseInt(id),
        produtoExistente,
      );
    } catch (error) {
      ...
      const estatisticas = {
        ...
        precoMedio:
          produtos.length > 0
            ...
            : 0,
        produtosComEstoqueBaixo: produtos.filter((p) => p.quantidade < 10)
          .length,
      };
