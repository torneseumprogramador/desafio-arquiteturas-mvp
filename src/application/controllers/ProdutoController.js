...
class ProdutoController {
  constructor(produtoService) {
    ...
    this.obterEstatisticasUseCase = new ObterEstatisticasUseCase(
      produtoService,
    );
  }
  ...
  async buscarProdutosPorPreco(req, res) {
    try {
      ...
      if (
        error.message.includes("Preços não podem ser negativos") ||
  ...
  async atualizarEstoque(req, res) {
    try {
      ...
      const produto = await this.gerenciarEstoqueUseCase.atualizarEstoque(
        id,
        parseInt(quantidade),
      );
      res.json(produto.toJSON());
  ...
  async aplicarDesconto(req, res) {
    try {
      ...
      const produto = await this.gerenciarEstoqueUseCase.aplicarDesconto(
        id,
