...
describe('MySQLProdutoRepository', () => {
  ...
  describe('criar', () => {
    it('deve criar produto com sucesso', async () => {
      const produto = new Produto(null, 'Notebook', 2500.00, 'Notebook Dell', 10);
      ...
      expect(mockConnection.query).toHaveBeenCalledWith(
        ...
        ['Notebook', 2500.00, 'Notebook Dell', 10],
        expect.any(Function)
      );
  ...
  describe('listarTodos', () => {
    it('deve listar todos os produtos', async () => {
      const produtosMock = [
        { id: 1, nome: 'Notebook', preco: 2500.00, descricao: 'Notebook Dell', quantidade: 10 },
        { id: 2, nome: 'Mouse', preco: 50.00, descricao: 'Mouse sem fio', quantidade: 20 }
      ];
  ...
  describe('buscarPorId', () => {
    it('deve buscar produto por ID com sucesso', async () => {
      const produtoMock = { id: 1, nome: 'Notebook', preco: 2500.00, descricao: 'Notebook Dell', quantidade: 10 };
  ...
  describe('atualizar', () => {
    it('deve atualizar produto com sucesso', async () => {
      const produto = new Produto(1, 'Notebook Atualizado', 2800.00, 'Notebook Dell Atualizado', 15);
      ...
      expect(mockConnection.query).toHaveBeenCalledWith(
        ...
        ['Notebook Atualizado', 2800.00, 'Notebook Dell Atualizado', 15, 1],
        expect.any(Function)
      );
    ...
    it('deve lançar erro quando produto não existe', async () => {
      const produto = new Produto(999, 'Notebook', 2500.00, 'Notebook Dell', 10);

      mockConnection.query.mockImplementation((sql, params, callback) => {
        callback(null, { affectedRows: 0 });
      });

      await expect(repository.atualizar(produto)).rejects.toThrow('Produto não encontrado');
      expect(mockConnection.release).toHaveBeenCalled();
    });
    ...
    it('deve lançar erro quando falha na atualização', async () => {
      ...
      await expect(repository.atualizar(produto)).rejects.toThrow('Erro de atualização');
      expect(mockConnection.release).toHaveBeenCalled();
    });
  });
  ...
  describe('excluir', () => {
    ...
    it('deve lançar erro quando produto não existe', async () => {
      ...
      await expect(repository.excluir(999)).rejects.toThrow('Produto não encontrado');
      expect(mockConnection.release).toHaveBeenCalled();
    });
  ...
  describe('gerenciarEstoque', () => {
    ...
    it('deve lançar erro quando produto não existe para gerenciar estoque', async () => {
      ...
      await expect(repository.gerenciarEstoque(999, 15)).rejects.toThrow('Produto não encontrado');
