...
describe('CriarProdutoUseCase', () => {
  ...
  describe('Executar', () => {
    it('deve criar um produto válido', async () => {
      ...
      const produtoCriado = new Produto(1, 'Notebook', 2500.00, 'Notebook Dell Inspiron', 10);
    ...
    it('deve validar dados antes de criar', async () => {
      ...
      await expect(useCase.executar(dadosInvalidos)).rejects.toThrow('Dados inválidos');
    ...
    it('deve rejeitar quando nome está vazio', async () => {
      ...
      await expect(useCase.executar(dadosProduto)).rejects.toThrow('Dados inválidos');
    });
    ...
    it('deve aceitar quantidade zero', async () => {
      ...
      const produtoCriado = new Produto(1, 'Notebook', 2500.00, 'Notebook Dell Inspiron', 0);
      mockRepository.criar.mockResolvedValue(produtoCriado);
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(useCase.executar(dadosProduto)).rejects.toThrow('Erro no banco de dados');
    });
    ...
    it('deve criar produto com dados mínimos', async () => {
      ...
      const produtoCriado = new Produto(1, 'Mouse', 50.00, '', 5);
      mockRepository.criar.mockResolvedValue(produtoCriado);
