...
describe('ObterEstatisticasUseCase', () => {
  ...
  describe('Executar', () => {
    ...
    it('deve calcular produtos com estoque baixo corretamente', async () => {
      const produtos = [
        new Produto(1, 'Notebook', 2500.00, 'Notebook Dell', 5), // estoque baixo
        new Produto(2, 'Mouse', 50.00, 'Mouse sem fio', 3), // estoque baixo
        new Produto(3, 'Teclado', 150.00, 'Teclado mecânico', 15) // estoque normal
      ];
    ...
    it('deve propagar erro do repositório na contagem', async () => {
      const produtos = [new Produto(1, 'Notebook', 2500.00, 'Notebook Dell', 10)];
      mockRepository.listarTodos.mockResolvedValue(produtos);
    ...
    it('deve calcular preço médio corretamente com valores decimais', async () => {
      const produtos = [
        new Produto(1, 'Produto 1', 100.50, 'Descrição 1', 5),
      ...
      expect(resultado.precoMedio).toBe((100.50 + 200.75) / 2); // 150.625
    });
