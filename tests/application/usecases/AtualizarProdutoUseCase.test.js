...
describe("AtualizarProdutoUseCase", () => {
  ...
  describe("Executar", () => {
    it("deve atualizar produto existente", async () => {
      ...
      expect(resultado.preco).toBe(2800.0);
    });
    ...
    it("deve lançar erro quando produto não existe", async () => {
      ...
      await expect(useCase.executar(999, dadosAtualizados)).rejects.toThrow(
    ...
    it("deve validar dados antes de atualizar", async () => {
      ...
      await expect(useCase.executar(1, dadosInvalidos)).rejects.toThrow(
        "Dados inválidos",
      );
      expect(mockRepository.atualizar).not.toHaveBeenCalled();
    });
    ...
    it("deve rejeitar quando nome está vazio", async () => {
      ...
      await expect(useCase.executar(1, dadosAtualizados)).rejects.toThrow(
        "Dados inválidos",
      );
    });
    ...
    it("deve lançar erro quando ID é inválido", async () => {
      ...
      await expect(useCase.executar(null, dadosAtualizados)).rejects.toThrow(
        "ID inválido",
      );
      await expect(
        useCase.executar(undefined, dadosAtualizados),
    ...
    it("deve atualizar apenas campos fornecidos", async () => {
      ...
      expect(resultado.preco).toBe(2500.0); // Mantém valor original
