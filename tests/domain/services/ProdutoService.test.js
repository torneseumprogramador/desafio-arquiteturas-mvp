...
describe('ProdutoService Domain', () => {
  ...
  describe('buscarProdutoPorId', () => {
    ...
    it('deve lançar erro quando ID é null', async () => {
      await expect(service.buscarProdutoPorId(null)).rejects.toThrow('ID inválido');
    });

    it('deve lançar erro quando ID é undefined', async () => {
      await expect(service.buscarProdutoPorId(undefined)).rejects.toThrow('ID inválido');
    });

    it('deve lançar erro quando ID não é número', async () => {
      await expect(service.buscarProdutoPorId('abc')).rejects.toThrow('ID inválido');
    });

    it('deve lançar erro quando ID é zero', async () => {
      await expect(service.buscarProdutoPorId(0)).rejects.toThrow('ID inválido');
    });
  ...
  describe('criarProduto', () => {
    ...
    it('deve validar dados antes de criar', async () => {
      ...
      await expect(service.criarProduto(dadosInvalidos)).rejects.toThrow('Dados inválidos');
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.criarProduto(dados)).rejects.toThrow('Erro no banco');
    });
  });
  ...
  describe('atualizarProduto', () => {
    ...
    it('deve manter valores originais quando não fornecidos', async () => {
      ...
      expect(mockRepository.atualizar).toHaveBeenCalledWith(1, expect.objectContaining({
        nome: 'Notebook Novo',
        preco: 2500.00,
        descricao: 'Notebook Dell',
        quantidade: 10
    ...
    it('deve lançar erro quando ID é inválido', async () => {
      await expect(service.atualizarProduto(null, {})).rejects.toThrow('ID inválido');
      await expect(service.atualizarProduto(undefined, {})).rejects.toThrow('ID inválido');
      await expect(service.atualizarProduto('abc', {})).rejects.toThrow('ID inválido');
    });
    ...
    it('deve validar dados atualizados', async () => {
      ...
      await expect(service.atualizarProduto(1, dadosInvalidos)).rejects.toThrow('Dados inválidos');
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.atualizarProduto(1, dadosAtualizados)).rejects.toThrow('Erro no banco');
    });
  });
  ...
  describe('excluirProduto', () => {
    ...
    it('deve lançar erro quando ID é inválido', async () => {
      ...
      await expect(service.excluirProduto(undefined)).rejects.toThrow('ID inválido');
      await expect(service.excluirProduto('abc')).rejects.toThrow('ID inválido');
    });
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.excluirProduto(1)).rejects.toThrow('Erro ao excluir');
    });
  });
  ...
  describe('buscarProdutosPorNome', () => {
    it('deve buscar produtos por nome válido', async () => {
      const produtos = [
        new Produto(1, 'Notebook Dell', 2500.00, 'Notebook Dell', 10),
        new Produto(2, 'Notebook HP', 2300.00, 'Notebook HP', 5)
      ];
    ...
    it('deve lançar erro quando nome está vazio', async () => {
      await expect(service.buscarProdutosPorNome('')).rejects.toThrow('Nome é obrigatório para busca');
      await expect(service.buscarProdutosPorNome('   ')).rejects.toThrow('Nome é obrigatório para busca');
    });

    it('deve lançar erro quando nome é null', async () => {
      await expect(service.buscarProdutosPorNome(null)).rejects.toThrow('Nome é obrigatório para busca');
    });

    it('deve lançar erro quando nome é undefined', async () => {
      await expect(service.buscarProdutosPorNome(undefined)).rejects.toThrow('Nome é obrigatório para busca');
    });
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.buscarProdutosPorNome('teste')).rejects.toThrow('Erro de busca');
    });
  });
  ...
  describe('buscarProdutosPorPreco', () => {
    it('deve buscar produtos por faixa de preço válida', async () => {
      const produtos = [
        new Produto(1, 'Mouse', 50.00, 'Mouse sem fio', 20),
        new Produto(2, 'Teclado', 150.00, 'Teclado mecânico', 15)
      ];
    ...
    it('deve lançar erro quando preço mínimo é undefined', async () => {
      await expect(service.buscarProdutosPorPreco(undefined, 100)).rejects.toThrow('Preço mínimo e máximo são obrigatórios');
    });

    it('deve lançar erro quando preço máximo é undefined', async () => {
      await expect(service.buscarProdutosPorPreco(10, undefined)).rejects.toThrow('Preço mínimo e máximo são obrigatórios');
    });

    it('deve lançar erro quando preço mínimo é negativo', async () => {
      await expect(service.buscarProdutosPorPreco(-10, 100)).rejects.toThrow('Preços não podem ser negativos');
    });
    ...
    it('deve lançar erro quando preço mínimo é maior que máximo', async () => {
      await expect(service.buscarProdutosPorPreco(200, 100)).rejects.toThrow('Preço mínimo não pode ser maior que o máximo');
    });
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.buscarProdutosPorPreco(10, 100)).rejects.toThrow('Erro de busca');
    });
  });
  ...
  describe('atualizarEstoque', () => {
    ...
    it('deve lançar erro quando ID é inválido', async () => {
      await expect(service.atualizarEstoque(null, 15)).rejects.toThrow('ID inválido');
      await expect(service.atualizarEstoque(undefined, 15)).rejects.toThrow('ID inválido');
      await expect(service.atualizarEstoque('abc', 15)).rejects.toThrow('ID inválido');
    });
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.atualizarEstoque(1, 15)).rejects.toThrow('Erro ao atualizar');
    });
  });
  ...
  describe('aplicarDesconto', () => {
    it('deve aplicar desconto com sucesso', async () => {
      const produto = new Produto(1, 'Notebook', 2500.00, 'Notebook Dell', 10);
      const produtoComDesconto = new Produto(1, 'Notebook', 2250.00, 'Notebook Dell', 10);
    ...
    it('deve lançar erro quando ID é inválido', async () => {
      await expect(service.aplicarDesconto(null, 10)).rejects.toThrow('ID inválido');
      await expect(service.aplicarDesconto(undefined, 10)).rejects.toThrow('ID inválido');
      await expect(service.aplicarDesconto('abc', 10)).rejects.toThrow('ID inválido');
    });
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.aplicarDesconto(1, 10)).rejects.toThrow('Erro ao aplicar desconto');
    });
  });
  ...
  describe('obterEstatisticas', () => {
    it('deve calcular estatísticas corretamente', async () => {
      const produtos = [
        new Produto(1, 'Notebook', 2500.00, 'Notebook Dell', 10),
        new Produto(2, 'Mouse', 50.00, 'Mouse sem fio', 20),
        new Produto(3, 'Teclado', 150.00, 'Teclado mecânico', 5)
      ];
    ...
    it('deve propagar erro do repositório', async () => {
      ...
      await expect(service.obterEstatisticas()).rejects.toThrow('Erro ao obter estatísticas');
    });
  });
}); 