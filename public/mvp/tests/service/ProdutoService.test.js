...
describe("ProdutoService", () => {
  ...
  it("deve lançar erro ao buscar produto inexistente", async () => {
    ...
    await expect(service.buscarProdutoPorId(999)).rejects.toThrow(
  ...
  it("deve lançar erro ao criar produto com falha", async () => {
    fetch.mockResolvedValueOnce({ ok: false });
    await expect(service.criarProduto({ nome: "Falha" })).rejects.toThrow(
  ...
  it("deve lançar erro ao atualizar produto com falha", async () => {
    fetch.mockResolvedValueOnce({ ok: false });
    await expect(service.atualizarProduto(1, { nome: "X" })).rejects.toThrow(
