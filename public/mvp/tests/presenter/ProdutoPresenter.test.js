...
describe("ProdutoPresenter", () => {
  ...
  beforeEach(() => {
    ...
    mockService = {
      ...
      buscarProdutoPorId: jest
        .fn()
        .mockResolvedValue({ id: 1, nome: "Editado" }),
  ...
  it("deve criar produto com sucesso", async () => {
    ...
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
  ...
  it("deve lançar erro ao criar produto", async () => {
    ...
    await expect(presenter.criarProduto({ nome: "Novo" })).rejects.toThrow(
      "Falha",
    );
  });
  ...
  it("deve atualizar produto com sucesso", async () => {
    ...
    expect(mockService.atualizarProduto).toHaveBeenCalledWith(1, {
      nome: "Atualizado",
    });
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
  ...
  it("deve lançar erro ao atualizar produto", async () => {
    presenter.produtoEmEdicao = { id: 1 };
    mockService.atualizarProduto.mockRejectedValueOnce(
      new Error("Falha update"),
    );
    await expect(presenter.atualizarProduto({ nome: "X" })).rejects.toThrow(
      "Falha update",
    );
  });
  ...
  it("deve buscar produto para edição", async () => {
    ...
    expect(mockView.preencherFormulario).toHaveBeenCalledWith({
      id: 1,
      nome: "Editado",
    });
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Editando produto: Editado",
      "info",
    );
  });

  it("deve mostrar erro ao buscar produto para edição", async () => {
    mockService.buscarProdutoPorId.mockRejectedValueOnce(
      new Error("Falha busca"),
    );
    await presenter.editarProduto(1);
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Erro ao carregar produto para edição: Falha busca",
      "error",
    );
  });
  ...
  it("deve validar dados e retornar erros", () => {
    const erros = presenter.validarDados({
      nome: "",
      preco: 0,
      quantidade: -1,
    });
  ...
  it("deve chamar setupEventListeners e handleProdutoSubmit", () => {
    const spy = jest
      .spyOn(presenter, "handleProdutoSubmit")
      .mockImplementation(() => {});
    const event = new CustomEvent("produtoSubmit", {
      detail: { nome: "Teste", preco: 1, quantidade: 1 },
    });
  ...
  it("deve mostrar erro ao inicializar", async () => {
    mockService.listarProdutos.mockRejectedValueOnce(
      new Error("Falha inicialização"),
    );
    await presenter.inicializar();
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Erro ao carregar produtos: Falha inicialização",
      "error",
    );
  });
  ...
  it("deve carregar produtos e mostrar erro", async () => {
    mockService.listarProdutos.mockRejectedValueOnce(
      new Error("Falha carregar"),
    );
    await presenter.carregarProdutos();
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Erro ao carregar produtos: Falha carregar",
      "error",
    );
  });
  ...
  it("deve handleProdutoSubmit com erro", async () => {
    ...
    jest
      .spyOn(presenter, "criarProduto")
      .mockRejectedValueOnce(new Error("Falha submit"));
    await presenter.handleProdutoSubmit({ nome: "X", preco: 1, quantidade: 1 });
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Erro ao salvar produto: Falha submit",
      "error",
    );
  });
  ...
  it("deve mostrar erro ao excluir produto", async () => {
    mockService.excluirProduto.mockRejectedValueOnce(
      new Error("Falha excluir"),
    );
    await presenter.excluirProduto(1);
    expect(mockView.mostrarMensagem).toHaveBeenCalledWith(
      "Erro ao excluir produto: Falha excluir",
      "error",
    );
  });
